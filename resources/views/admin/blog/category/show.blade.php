	                ...
	                <tbody>
	                	@foreach($categories as $category)
			                <tr>
			                  	...
			                  	<td>{{$category->slug}}</td>
			                  	<td>
		                            <a href="{{route('post-category.edit',$category->id)}}" style=" font-size: 18px;"><i class="fa fa-edit"></i></a>
		                        </td>
		                        <td>
		                            <form action="{{route('post-category.destroy',$category->id)}}" method="post" id="delete-form-{{$category->id}}" style="display: none;">
		                              {{csrf_field()}}
		                              {{method_field('DELETE')}}
		                            </form>
